...
contract('Dex', (accounts) => {
  ...
  it('should NOT deposit tokens unregistered in dex', async () => {
    const amount = web3.utils.toWei('100');
    await expectRevert(
      dex.deposit(amount, web3.utils.fromAscii('UNREGISTERED-TOKEN'), {
        from: trader1,
      }),
  ...
  it('should NOT withdraw tokens unregistered in dex', async () => {
    const amount = web3.utils.toWei('100');
    await expectRevert(
      dex.withdraw(amount, web3.utils.fromAscii('UNREGISTERED-TOKEN'), {
        from: trader1,
      }),
      'this token does not exists'
    );
  ...
  it('should NOT create limit order if token balance is too low', async () => {
    ...
    await dex.deposit(amount, REP, { from: trader1 });
    await expectRevert(
  ...
  it('should create market order & match against existing limit order', async () => {
    ...
    await dex.deposit(amount, REP, { from: trader2 });
